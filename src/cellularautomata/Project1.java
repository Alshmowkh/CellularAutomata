...
/**
 * Paints the Cellular Automaton specified by the ruleSet variable onto the
 * screen.
 */
public class Project1 extends javax.swing.JPanel {
    ...
    public void calculate() {
        ...
        do {
            if ((ruleSet & ~(1 << (current
                    += ((getList(!first).contains(p - 1) ? 4 : 0) + (getList(!first).contains(p) ? 2 : 0) + (getList(!first).contains(p + 1) ? 1 : 0))))) == ruleSet) {
                getList(first).add(p);
            }
            current = 0;
            beg -= (beg == p) ? 1 : 0;
